...
$StdOut = Join-Path $LogDir "bp1d_mcts_recipe_$Timestamp.log"
$StdErr = Join-Path $LogDir "bp1d_mcts_recipe_${Timestamp}_error.log"
...
$LaunchJson = Join-Path $LogDir "bp1d_mcts_recipe_${Timestamp}_launch.json"
$LaunchInfo | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $LaunchJson -Encoding UTF8

Write-Host "Started BP_1D Recipe-MCTS background run."
