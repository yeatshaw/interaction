...
$StdOut = Join-Path $LogDir "kp_mcts_recipe_$Timestamp.log"
$StdErr = Join-Path $LogDir "kp_mcts_recipe_${Timestamp}_error.log"
...
$LaunchJson = Join-Path $LogDir "kp_mcts_recipe_${Timestamp}_launch.json"
$LaunchInfo | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $LaunchJson -Encoding UTF8

Write-Host "Started KP Recipe-MCTS background run."
