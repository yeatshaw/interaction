"""Run single-path adaptive-recipe EoH on constructive TSP."""
...
from example.tasks.adaptive_recipe_eoh_runner import run_task
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run single-path adaptive-recipe EoH on TSP."
    )
    parser.add_argument(
        "--train-data",
        default=os.environ.get("LLM4AD_TSP_TRAIN_DATA"),
        help=("Path to a pickle training dataset. Random instances are "
              "generated when neither this option nor "
              "LLM4AD_TSP_TRAIN_DATA is provided."),
    )
...
def main():
    ...
    run_task(
        ...
        llm=llm,
        log_dir="logs/adaptive_recipe_eoh_tsp",
    )
